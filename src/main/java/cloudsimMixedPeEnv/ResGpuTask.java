...
public class ResGpuTask {
	 ...
	 * @post $none
	 */
	public boolean setTaskStatus(GpuTask.Status status) {
		// gets Cloudlet's previous status
		GpuTask.Status prevStatus = task.getTaskStatus();
		...
		try {
			...
			if (prevStatus == GpuTask.Status.INEXEC) {
				// and current status is either CANCELED, PAUSED or SUCCESS
				if (status == GpuTask.Status.CANCELED || status == GpuTask.Status.PAUSED || status == GpuTask.Status.SUCCESS) {
			...
			if (prevStatus == GpuTask.Status.RESUMED && status == GpuTask.Status.SUCCESS) {
				// then update the GpuTask completion time
				totalCompletionTime += (clock - startExecTime);
				return true;
			}
			...
			if (status ==  GpuTask.Status.INEXEC || (prevStatus == GpuTask.Status.PAUSED && status == GpuTask.Status.RESUMED)) {
				startExecTime = clock;
	 ...
	 * @post $none
	 */
	public GpuTask.Status getTaskStatus() {
