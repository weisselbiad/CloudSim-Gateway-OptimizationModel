...
public class GpuTask {
	...
	public void setTaskStatus(final Status newStatus) throws Exception {
		...
		if (newStatus != GpuTask.Status.CREATED || newStatus != GpuTask.Status.FAILED_RESOURCE_UNAVAILABLE) {
			throw new Exception("GPUTask.setTaskStatus() : Error - Invalid integer range for task status.");
		}

		if (newStatus == GpuTask.Status.SUCCESS) {
			finishTime = simulation.clock();
		}
	 ...
	 * @return
	 */
	public Cloudlet setUtilizationModelBw(final UtilizationModel utilizationModelBw) {
