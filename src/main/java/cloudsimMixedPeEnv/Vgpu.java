...
public class Vgpu {
	...
	public Vgpu(CloudSim simulation, int vgpuId, double mips, int numberOfPes, int gddram, long bw, String type, String tenancy,
			GpuTaskScheduler scheduler, int PCIeBw) {
		this.simulatiion = simulation;
	...
	public List<Double> getCurrentUtilizationOfGddram() {
		...
		for (ResGpuTask rcl : getGpuTaskScheduler().getTaskExecList()) {
			gddramUtilization.add(rcl.getGpuTask().getUtilizationOfGddram(simulatiion.clock()));
		}
